...
alldat <- readRDS("data/data-loaded-Nov19.rds")
...
for (mod_id in seq_along(var_sets)) {
  ...
  # saveRDS(mod, file = paste0("outputs/fitted/full-model-test-yoy-murraywith5yo.rds"))
  
}

# validate model
# mod$mcmc_settings$n_samples <- 10000
# mod$mcmc_settings$warmup <- 25000
# mod_cv <- validate(mod, folds = 10, thin = 12)
